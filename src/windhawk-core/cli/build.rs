//! Build hook: embed a Windows VERSIONINFO resource into windhawk-cli.exe so the
//! shipped file carries the app version and the Ramen Software / Windhawk product
//! strings in its Details tab, matching the version info the Tauri UI already
//! embeds in windhawk-ui.exe. The .rc is generated from Cargo's version env vars
//! and compiled by the Windows SDK RC.EXE via embed-resource.
...
fn main() {
    ...
    // Link the resource only into the front-end binary (the crate also builds
    // the src/bin/ compiler + schtasks test stubs, which stay unbranded).
    // manifest_required() turns a missing RC.EXE or a failed compile into a hard
    // build error, so a shipped exe never silently loses its version info.
    embed_resource::compile_for(&rc_path, ["windhawk-cli"], embed_resource::NONE)
        .manifest_required()
