...
import { debugIgnoreCertErrors } from '../storage/debugOverrides';
import { RepoUnreachableError } from './errors';

type ArchitectureSubfolder = '32' | '64' | 'arm64';

export default class ModFiles {
	...
	public async downloadPrecompiledMod(
		...
		// Create HTTP agent with keepAlive to reuse connections. Debug-only:
		// WINDHAWK_DEBUG_IGNORE_CERT_ERRORS relaxes TLS validation so a
		// self-signed test repo can be used (production builds drop the branch
		// and keep the default strict validation).
		const agent = new https.Agent({
			keepAlive: true,
			rejectUnauthorized: !debugIgnoreCertErrors(),
		});
		...
		for (let i = 0; i < downloads.length; i++) {
			...
			if (!response.ok) {
				throw new RepoUnreachableError(
					`Failed to download ${downloads[i].subfolder} DLL: ${response.statusText || response.status}`,
				);
			}
		}
		...
		if (!versionsJsonResponse.ok) {
			throw new RepoUnreachableError(
				`Failed to download versions.json: ${versionsJsonResponse.statusText || versionsJsonResponse.status}`,
			);
		}
		...
		if (minWindhawkVersion) {
			...
			const requiredVersion = semver.coerce(minWindhawkVersion, { includePrerelease: true });
