...
import { WindhawkError } from './errors';
...
export default class Compiler {
	...
	private windhawkVersionHex: string;
	...
	// Per-target DLL paths the in-flight compileMod() is producing. Read by
	// cancelCompilation() to unlink files whose compile it just interrupted.
	private pendingDllPaths: Set<string> = new Set();

	public constructor(compilerPath: string, enginePath: string, appDataPath: string, arm64Enabled: boolean, currentWindhawkVersion: semver.SemVer | null, logger: Logger) {
		...
		if (currentWindhawkVersion) {
			const { major, minor, patch } = currentWindhawkVersion;
			this.windhawkVersionHex = '0x' +
				major.toString(16).padStart(2, '0') +
				minor.toString(16).padStart(2, '0') +
				patch.toString(16).padStart(2, '0') +
				'00';
		} else {
			this.windhawkVersionHex = '0x00000000';
		}
	...
	private async makePrecompiledHeaders(
		...
		const args = [
			...
			`-DWH_WINDHAWK_VERSION=${this.windhawkVersionHex}`,
			'-x',
			'c++-header',
			pchHeaderPath,
			'-target',
	...
	public async compileMod(
		...
		try {
			for (const target of this.compilationTargetsFromArchitecture(architectures, modTargets)) {
				...
				if (precompiledHeadersFolder) {
					...
					if (fs.existsSync(pchHeaderPath)) {
						...
						if (!fs.existsSync(pchPath) ||
							...
							if (stdout) {
								console.warn(`Precompiled headers stdout for target ${target}:\n${stdout}`);
							}
							if (stderr) {
								console.warn(`Precompiled headers stderr for target ${target}:\n${stderr}`);
							}
						}
					}
				}

				this.pendingDllPaths.add(
					path.join(this.engineModsPath, this.subfolderFromCompilationTarget(target), targetDllName),
				);
				...
				if (stdout) {
					console.warn(`Compiler stdout for target ${target}:\n${stdout}`);
				}
				if (stderr) {
					console.warn(`Compiler stderr for target ${target}:\n${stderr}`);
				}
			}
		} finally {
			// Drop tracking: on successful return the caller owns the DLLs; on
			// a thrown CompilerError the files remain for the next compileMod's
			// deleteOldModFiles sweep. Only cancelCompilation should unlink
			// from this set.
			this.pendingDllPaths.clear();
		}
	...
	public cancelCompilation() {
		...
		// Best-effort synchronous unlink of the DLLs the killed compile was
		// writing. Sync so this method completes the cleanup before returning.
		// unlinkSync can fail if Windows hasn't released the child's file
		// handle yet or the file was never created - either way, leave it for
		// deleteOldModFiles to sweep later.
		for (const dllPath of this.pendingDllPaths) {
			try {
				fs.unlinkSync(dllPath);
			} catch {
				// Ignore: file missing or still locked.
			}
		}
		this.pendingDllPaths.clear();
