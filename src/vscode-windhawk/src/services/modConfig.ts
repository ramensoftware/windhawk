...
import { ModConfig, ModSettings } from './types';
...
// Map each TypeScript primitive used in ModConfig to a descriptor tag.
type FieldTypeTag<T> =
...
// Keyed by every ModConfig field: the `satisfies` clause forces the literal
// to cover every key of ModConfig AND forces each entry's `type` tag to
// match that key's declared TypeScript type. Adding a ModConfig field
// without updating CONFIG_FIELDS (or mistyping a tag) is a compile error
// at this declaration.
const CONFIG_FIELDS = {
	libraryFileName: { storageName: 'LibraryFileName', type: 'string' },
	disabled: { storageName: 'Disabled', type: 'boolean' },
	loggingEnabled: { storageName: 'LoggingEnabled', type: 'boolean' },
	debugLoggingEnabled: { storageName: 'DebugLoggingEnabled', type: 'boolean' },
	include: { storageName: 'Include', type: 'string-array' },
	exclude: { storageName: 'Exclude', type: 'string-array' },
	includeCustom: { storageName: 'IncludeCustom', type: 'string-array' },
	excludeCustom: { storageName: 'ExcludeCustom', type: 'string-array' },
	includeExcludeCustomOnly: { storageName: 'IncludeExcludeCustomOnly', type: 'boolean' },
	patternsMatchCriticalSystemProcesses: { storageName: 'PatternsMatchCriticalSystemProcesses', type: 'boolean' },
	architecture: { storageName: 'Architecture', type: 'string-array' },
	version: { storageName: 'Version', type: 'string' },
} as const satisfies { [K in keyof ModConfig]: { storageName: string; type: FieldTypeTag<ModConfig[K]> } };

type ConfigFieldName = keyof typeof CONFIG_FIELDS;
type ConfigFieldDescriptor = typeof CONFIG_FIELDS[ConfigFieldName];
type StorageFieldName = ConfigFieldDescriptor['storageName'];

// Flat, distributive entry shape: name, storageName, type live on the same
// object so that switching on `type` narrows `name` too. Tuples + destructuring
// lose this correlation.
type ConfigFieldEntry = {
	[K in ConfigFieldName]: {
		name: K;
		storageName: typeof CONFIG_FIELDS[K]['storageName'];
		type: typeof CONFIG_FIELDS[K]['type'];
	};
}[ConfigFieldName];

function configFieldEntries(): ConfigFieldEntry[] {
	return Object.entries(CONFIG_FIELDS).map(
		([name, desc]) => ({ name, ...desc })
	) as ConfigFieldEntry[];
}
...
class ModConfigCodec {
	static parse(backend: ModStorageBackend, modId: string): ModConfig | null {
		...
		for (const field of configFieldEntries()) {
...
class IniStorageBackend implements ModStorageBackend {
	...
	readAllConfigFields(modId: string): Partial<Record<StorageFieldName, string | number>> | null {
		...
		for (const field of Object.values(CONFIG_FIELDS)) {
...
export interface ModConfigService {
	getConfigOfInstalled(): Record<string, ModConfig>;
...
class ModConfigBase implements ModConfigService {
...
export class ModConfigPortable extends ModConfigBase {
	public constructor(appDataPath: string) {
		super(new IniStorageBackend(appDataPath));
	}
}

export class ModConfigNonPortable extends ModConfigBase {
