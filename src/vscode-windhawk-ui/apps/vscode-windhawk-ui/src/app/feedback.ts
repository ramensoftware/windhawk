...
/**
 * The machine-readable error a reply carries on a command failure (mirrors the Rust
 * `reply::error_object`). `code` is a stable SCREAMING_SNAKE string; `location` is
 * the optional source origin (DIAGNOSTIC), shown human-facing only.
 */
