...
function About() {
  ...
  const { updateIsAvailable, updateIsAvailableBleedingEdge } = useContext(AppUISettingsContext);

  // Trim trailing ".0" groups from the numeric base and render any pre-release
  // tag readably: "2.0.0" -> "2.0", "2.0.0-alpha.1" -> "2.0 alpha 1".
  const rawVersion = process.env['REACT_APP_VERSION'] || 'unknown';
  const dashIndex = rawVersion.indexOf('-');
  const baseVersion = dashIndex === -1 ? rawVersion : rawVersion.slice(0, dashIndex);
  const prerelease = dashIndex === -1 ? '' : rawVersion.slice(dashIndex + 1);
  const currentVersion =
    baseVersion.replace(/^(\d+(?:\.\d+)+?)(\.0+)+$/, '$1') +
    (prerelease ? ` ${prerelease.replace(/\./g, ' ')}` : '');

  return (
    <AboutContainer data-testid="about-page">
      <AboutContent>
        <ContentSection>
          <h1>
            {t('about.title', {
        ...
        {(updateIsAvailable || updateIsAvailableBleedingEdge) && (
          <ContentSection>
            <Alert
