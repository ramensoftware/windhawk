...
const Root = styled.div<{ $visible: boolean; $height: number }>`
  ...
  background: var(--whui-background-color);
  /* Sit above the two things the pane docks over. The main content's scroll
     region is position:relative, so it paints in the positioned phase and would
     otherwise draw its overlay scrollbar over this (static) pane. The floating
     "Create a new mod" button is position:fixed at z-index 100, so the pane must
     outrank it to cover it rather than let it hover above the log. */
  position: relative;
  z-index: 101;
`;
